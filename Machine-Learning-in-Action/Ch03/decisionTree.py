'''
This is my own practice for decision tree——ID3

part I:
data,featureNames=createDataSet()
Entropy(data)
filterDataset(data,0,1)
chooseBestFeature(data)

data,featureNames=createDataSet()
deciTree=createDecisionTree(data,featureNames)
classifyDecisionTree(deciTree,featureNames,[1,0])
classifyDecisionTree(deciTree,featureNames,[1,1])

和 sklearn.tree 比较
因为sklearn中的训练集中不能含有类标签，所以要去掉,
另外，类标签Y也只能是整数，不能是string，所以也要做转换
from sklearn.tree import DecisionTreeClassifier
dataTrain=[entry[:-1] for entry in data]
d=dict([['yes',1],['no',0]])
dataClass=[d[label[-1]] for label in data]
clf=DecisionTreeClassifier()
clf.fit(dataTrain,dataClass)
