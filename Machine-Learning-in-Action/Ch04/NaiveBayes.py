...
def trainMultinomialNB(dataArray, classLabels):
    ...
    :return:
    pyPositive:一个数，表示P(Y=1)的类先验概率
    1-pyPositive: 表示P(Y=0)的类先验概率
    py1ConditionVec:一个向量，array类型，是P(Xi|Y=1)的概率向量
    py0ConditionVec:也是一个向量，array类型，是P(Xi|Y=0)的概率向量
    """
    # 首先计算训练实例个数，也就是行数
    numOfDocs=dataArray.shape[0]
    ...
    return pyPositive, 1-pyPositive, py1ConditionVec, py0ConditionVec
